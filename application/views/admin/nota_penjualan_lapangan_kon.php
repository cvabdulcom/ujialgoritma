        ...
        <ul class="treeview-menu">
          <li class="active treeview menu-open">
            ...
            <ul class="treeview-menu">
              ...
              <li class="active"><a href="<?php echo base_url('penjualan/notakon'); ?>"><i class="fa fa-circle-o"></i> Konsinyasi</a></li>
            </ul>
          </li>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1>
      <b>Nota Penjualan</b>
      <small>Lapangan (Konsinyasi)</small>
    </h1>
    <ol class="breadcrumb">
...
<script>
  ...
  var mycart_stokOutlet_kon = [];
    $(function () {
      if (localStorage.mycart){
        mycart_penjualan_lapangan = JSON.parse(localStorage.mycart_penjualan_lapangan);
        mycart_stokOutlet_kon = JSON.parse(localStorage.mycart_stokOutlet_kon);
        showCart();
      }
    });
  ...
  $('.add').click(function(){
    ...
    var tipe_pembelian_add = "KONSINYASI";
  ...
  function Refresh(){
    ...
    localStorage.removeItem("mycart_stokOutlet_kon");
    clearCart()
  }
  ...
  function stokOutlet(id_outlet_add, tanggal_add, id_produk_add, keluar_add, jumlah_add, no_nota_penjualan_add, id_file_penjualan_add) {
    //cek data in cart then update qty
    for (var i in mycart_stokOutlet_kon) {
      if(mycart_stokOutlet_kon[i].id_produk == id_produk_add)
      {
    ...
    mycart_stokOutlet_kon.push(item);
    
    if( window.localStorage){
      localStorage.mycart_stokOutlet_kon = JSON.stringify(mycart_stokOutlet_kon);
    }
  }

  function deleteItem(index){
    mycart_penjualan_lapangan.splice(index,1); // hapus item berdasarkan index
    mycart_stokOutlet_kon.splice(index,1);
    saveCart();
    showCart();
  }
  ...
  function Simpan_piutang(){
    ...
    function Simpan_stokoutlet(){
      if(window.localStorage!==undefined) {
        $.ajax({
            type: "POST",
            url: "<?php echo base_url('suratjalan/Simpan_stokoutlet_kon'); ?>",
            data: {data: localStorage.getItem('mycart_stokOutlet_kon')},
            dataType: "json",
          success: function(cek){
