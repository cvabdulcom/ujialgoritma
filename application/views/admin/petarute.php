...
<script>
  var markers = [
    <?php
      ...
      {
        echo '["'.$row['nama_outlet'].'", "'.$row['kabupaten'].'", "'.$row['alamat'].'", "'.$row['longitude'].'", "'.$row['latitude'].'", "'.$row['nama_salesman'].'", "'.$row['status_program'].'"],';
      }
    ?>
  ];
  ...
  function initialize() {
    ...
    for (i = 0; i < markers.length; i++) {  
      ...
      var sales = markers[i][5];
      var status = markers[i][6];
      
      gambar = '<?php echo base_url(); ?>/assets/image/'+sales.split(' ').join('')+''+status.split(' ').join('')+'.png';
      ...
      google.maps.event.addListener(marker, 'click', (function(marker, i) {
        return function() {
          var dataoutlet = "<b>" +markers[i][0]+ "</b> - <a href='http://www.google.com/maps/place/"+markers[i][3]+","+markers[i][4]+"' target='_blank'>Share Location</a> <br/>" +markers[i][2]+ ", " +markers[i][1]+ "<br/>";
      ...
      <div class="col-md-12">
        <div class="nav-tabs-custom">
          <ul class="nav nav-tabs">
            <li class="active"><a href="<?php echo base_url('petarute'); ?>">By Sales</a></li>
            <li><a href="<?php echo base_url('petarute/by_rute'); ?>">By Rute</a></li>
          </ul>
