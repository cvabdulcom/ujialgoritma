      ...
      <div class="col-md-12">
        ...
        <div class="nav-tabs-custom">
          <ul class="nav nav-tabs">
            ...
            <li class="active"><a href="<?php echo base_url('laporan/stok_global'); ?>">Global</a></li>
              ...
              <div class="form-group">
                <div class="input-group col-md-3">
                  ...
                  <span class="input-group-btn">
                    <form method="get" action="<?php echo base_url('laporan/stok_global'); ?>">
              ...
              <table id="example4" class="table table-bordered table-hover">
                ...
                <tr>
                  ...
                  <td class="text-center">
                    <?php
                        $cek_pcs_masuk_outlet = $this->db->query("SELECT SUM(tbl_stokoutlet.jumlah) AS jumlah
                        ...
                        $pcs_masuk_outlet = $cek_pcs_masuk_outlet->row()->jumlah;

                        $cek_pcs_masuk_gudang = $this->db->query("SELECT SUM(tbl_stokgudang.jumlah) AS jumlah
                          FROM tbl_stokgudang 
                          WHERE tbl_stokgudang.id_produk = '$id_produk' 
                          AND tbl_stokgudang.masuk_keluar = 'MASUK'
                          AND tbl_stokgudang.tanggal < '$tanggal_awal'");
                        $pcs_masuk_gudang = $cek_pcs_masuk_gudang->row()->jumlah;


                        $cek_pcs_keluar_outlet = $this->db->query("SELECT SUM(tbl_stokoutlet.jumlah) AS jumlah
                          FROM tbl_stokoutlet, tbl_outlet 
                          WHERE tbl_stokoutlet.id_outlet = tbl_outlet.id_outlet
                          AND tbl_outlet.tipe_pembelian='K'
                          AND tbl_stokoutlet.id_produk = '$id_produk' 
                          ...
                          AND tbl_stokoutlet.tanggal < '$tanggal_awal'");
                        $pcs_keluar_outlet = $cek_pcs_keluar_outlet->row()->jumlah;

                        $cek_pcs_keluar_gudang = $this->db->query("SELECT SUM(tbl_stokgudang.jumlah) AS jumlah
                          FROM tbl_stokgudang 
                          WHERE tbl_stokgudang.id_produk = '$id_produk' 
                          AND tbl_stokgudang.masuk_keluar = 'KELUAR'
                          AND tbl_stokgudang.tanggal < '$tanggal_awal'");
                        $pcs_keluar_gudang = $cek_pcs_keluar_gudang->row()->jumlah;

                        $pcs_masuk = $pcs_masuk_outlet+$pcs_masuk_gudang;
                        $pcs_keluar = $pcs_keluar_outlet+$pcs_keluar_gudang;
                  ...
                  <td class="text-center">
                    <?php
                      $cek_rph_masuk_outlet = $this->db->query("SELECT SUM(tbl_stokoutlet.jumlah) AS jumlah
                        FROM tbl_stokoutlet, tbl_outlet 
                        WHERE tbl_stokoutlet.id_outlet = tbl_outlet.id_outlet
                        AND tbl_outlet.tipe_pembelian='K'
                        AND tbl_stokoutlet.id_produk = '$id_produk' 
                        AND tbl_stokoutlet.masuk_keluar = 'MASUK'
                        AND tbl_stokoutlet.tanggal < '$tanggal_awal'");
                      $rph_masuk_outlet = $cek_rph_masuk_outlet->row()->jumlah*$harga_beli;

                      $cek_rph_masuk_gudang = $this->db->query("SELECT SUM(tbl_stokgudang.jumlah) AS jumlah
                        FROM tbl_stokgudang 
                        WHERE tbl_stokgudang.id_produk = '$id_produk' 
                        AND tbl_stokgudang.masuk_keluar = 'MASUK'
                        AND tbl_stokgudang.tanggal < '$tanggal_awal'");
                      $rph_masuk_gudang = $cek_rph_masuk_gudang->row()->jumlah*$harga_beli;


                      $cek_rph_keluar_outlet = $this->db->query("SELECT SUM(tbl_stokoutlet.jumlah) AS jumlah
                        FROM tbl_stokoutlet, tbl_outlet 
                        WHERE tbl_stokoutlet.id_outlet = tbl_outlet.id_outlet
                        AND tbl_outlet.tipe_pembelian='K'
                        AND tbl_stokoutlet.id_produk = '$id_produk' 
                        AND tbl_stokoutlet.masuk_keluar = 'KELUAR'
                        AND tbl_stokoutlet.tanggal < '$tanggal_awal'");
                      $rph_keluar_outlet = $cek_rph_keluar_outlet->row()->jumlah*$harga_beli;

                      $cek_rph_keluar_gudang = $this->db->query("SELECT SUM(tbl_stokgudang.jumlah) AS jumlah
                        FROM tbl_stokgudang 
                        WHERE tbl_stokgudang.id_produk = '$id_produk' 
                        AND tbl_stokgudang.masuk_keluar = 'KELUAR'
                        AND tbl_stokgudang.tanggal < '$tanggal_awal'");
                      $rph_keluar_gudang = $cek_rph_keluar_gudang->row()->jumlah*$harga_beli;

                      $rph_masuk = $rph_masuk_outlet+$rph_masuk_gudang;
                      $rph_keluar = $rph_keluar_outlet+$rph_keluar_gudang;
                  ...
                  <td class="text-center">
                    <?php
                        $masuk_cek_pcs_outlet = $this->db->query("SELECT SUM(tbl_stokoutlet.jumlah) AS jumlah
                          FROM tbl_stokoutlet, tbl_outlet 
                          WHERE tbl_stokoutlet.id_outlet = tbl_outlet.id_outlet
                          AND tbl_outlet.tipe_pembelian='K'
                          AND tbl_stokoutlet.id_produk = '$id_produk' 
                        ...
                        $masuk_pcs_outlet = $masuk_cek_pcs_outlet->row()->jumlah;

                        $masuk_cek_pcs_gudang = $this->db->query("SELECT SUM(tbl_stokgudang.jumlah) AS jumlah
                          FROM tbl_stokgudang 
                          WHERE tbl_stokgudang.id_produk = '$id_produk' 
                          AND tbl_stokgudang.masuk_keluar = 'MASUK'
                          AND tbl_stokgudang.tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir'");
                        $masuk_pcs_gudang = $masuk_cek_pcs_gudang->row()->jumlah;

                        $masuk_pcs = $masuk_pcs_outlet+$masuk_pcs_gudang;
                        echo round($masuk_pcs,2);
                      ?>
                  </td>
                  <td class="text-center">
                    <?php
                      $masuk_cek_rph_outlet = $this->db->query("SELECT SUM(tbl_stokoutlet.jumlah) AS jumlah
                        FROM tbl_stokoutlet, tbl_outlet 
                        WHERE tbl_stokoutlet.id_outlet = tbl_outlet.id_outlet
                        AND tbl_outlet.tipe_pembelian='K'
                        AND tbl_stokoutlet.id_produk = '$id_produk' 
                        AND tbl_stokoutlet.masuk_keluar = 'MASUK'
                        AND tbl_stokoutlet.tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir'");
                      $masuk_rph_outlet = $masuk_cek_rph_outlet->row()->jumlah*$harga_beli;

                      $masuk_cek_rph_gudang = $this->db->query("SELECT SUM(tbl_stokgudang.jumlah) AS jumlah
                        FROM tbl_stokgudang 
                        WHERE tbl_stokgudang.id_produk = '$id_produk' 
                        AND tbl_stokgudang.masuk_keluar = 'MASUK'
                        AND tbl_stokgudang.tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir'");
                      $masuk_rph_gudang = $masuk_cek_rph_gudang->row()->jumlah*$harga_beli;

                      $masuk_rph = $masuk_rph_outlet+$masuk_rph_gudang;
                  ...
                  <td class="text-center">
                    <?php
                        $keluar_cek_pcs_outlet = $this->db->query("SELECT SUM(tbl_stokoutlet.jumlah) AS jumlah
                          FROM tbl_stokoutlet, tbl_outlet 
                          WHERE tbl_stokoutlet.id_outlet = tbl_outlet.id_outlet
                          AND tbl_outlet.tipe_pembelian='K'
                          AND tbl_stokoutlet.id_produk = '$id_produk' 
                          AND tbl_stokoutlet.masuk_keluar = 'KELUAR'
                          AND tbl_stokoutlet.tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir'");
                        $keluar_pcs_outlet = $keluar_cek_pcs_outlet->row()->jumlah;

                        $keluar_cek_pcs_gudang = $this->db->query("SELECT SUM(tbl_stokgudang.jumlah) AS jumlah
                          FROM tbl_stokgudang 
                          WHERE tbl_stokgudang.id_produk = '$id_produk' 
                          AND tbl_stokgudang.masuk_keluar = 'KELUAR'
                          AND tbl_stokgudang.tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir'");
                        $keluar_pcs_gudang = $keluar_cek_pcs_gudang->row()->jumlah;
                        
                        $keluar_pcs = $keluar_pcs_outlet+$keluar_pcs_gudang;
                        echo round($keluar_pcs,2);
                      ?>
                  </td>
                  <td class="text-center">
                    <?php
                      $keluar_cek_rph_outlet = $this->db->query("SELECT SUM(tbl_stokoutlet.jumlah) AS jumlah
                        FROM tbl_stokoutlet, tbl_outlet 
                        WHERE tbl_stokoutlet.id_outlet = tbl_outlet.id_outlet
                        AND tbl_outlet.tipe_pembelian='K'
                        AND tbl_stokoutlet.id_produk = '$id_produk' 
                        AND tbl_stokoutlet.masuk_keluar = 'KELUAR'
                        AND tbl_stokoutlet.tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir'");
                      $keluar_rph_outlet = $keluar_cek_rph_outlet->row()->jumlah*$harga_beli;

                      $keluar_cek_rph_gudang = $this->db->query("SELECT SUM(tbl_stokgudang.jumlah) AS jumlah
                        FROM tbl_stokgudang 
                        WHERE tbl_stokgudang.id_produk = '$id_produk' 
                        AND tbl_stokgudang.masuk_keluar = 'KELUAR'
                        AND tbl_stokgudang.tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir'");
                      $keluar_rph_gudang = $keluar_cek_rph_gudang->row()->jumlah*$harga_beli;

                      $keluar_rph = $keluar_rph_outlet+$keluar_rph_gudang;
