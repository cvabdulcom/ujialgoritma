    ...
    <ul class="sidebar-menu" data-widget="tree">
      ...
      <li class="active treeview menu-open">
        ...
        <ul class="treeview-menu">
          ...
          <li class="active"><a href="<?php echo base_url('hapusdata/datasetoran'); ?>"><i class="fa fa-circle-o"></i> Data Setoran</a></li>
        </ul>
      </li>
    </ul>
  </section>
          ...
          <div class="box-footer">
            ...
            <div class="input-group pull-right form-horizontal col-md-3">
              ...
              <span class="input-group-btn">
                <form method="get" action="<?php echo base_url('hapusdata/datasetoran'); ?>">
          ...
          <div class="box-body table-responsive">
            <table id="example2" class="table table-bordered table-striped dataTable">
              ...
              <tbody>
                <?php
                  $no=0;
                  foreach($datasetoran->result_array() as $tampil):
                    $no++;
                    $id_kas = $tampil['id_kas'];
                    $tanggal = $tampil['tanggal'];
                    $total_uang_masuk_cash = $tampil['total_uang_masuk_cash'];
                    $total_uang_masuk_bank = $tampil['total_uang_masuk_bank'];
                    $total_uang_keluar_cash = $tampil['total_uang_keluar_cash'];
                    $total_uang_keluar_bank = $tampil['total_uang_keluar_bank'];
                    $setor = $tampil['setor'];
                    $id_file = $tampil['id_file'];
                    $bank = $tampil['bank'];
                ?>
                <tr>                  
                  <td class="text-center"><?php echo $no; ?></td>
                  <td class="text-center"><?php echo $id_kas; ?></td>
                  <td class="text-center"><?php $new = strtotime($tanggal); $newDate = date('d/m/Y', $new);  echo $newDate; ?></td>
                  <td class="text-center"><?php echo number_format($setor, 0, ',', '.'); ?></td>
                  <td class="text-left"><?php echo $bank; ?></td>
                  <td class="text-left">
                    <button class="btn btn-danger btn-xs" data-toggle="modal" data-target="#HAPUS<?php echo $id_kas; ?>">Hapus</button>  
                    <!-- /.modal -->
                    <form method="POST" action="<?php echo base_url('hapusdata/hapus_setoran'); ?>">
                      <div class="modal fade bd-example-modal-sm" id="HAPUS<?php echo $id_kas; ?>">
                        <div class="modal-dialog modal-sm">
                          <div class="modal-content">
                            <div class="modal-body">
                              Anda yakin hapus setoran <strong><?php echo $id_kas; ?></strong> ?
                            </div>
